...
class Nine_Parser(object):
    ...
    def parse(self, tokens : List[Token]) -> List[Instruction]:
        ends = self.parse_end_instructions(tokens)
        begins = self.parse_begin_instructions(ends)
        return self.parse_to_instruction(begins)
    ...
    def parse_begin_instructions(self, tokens : List[List[Token]]) -> List[Token]:
        ...
        tail = tokens[head_count + 1:] #not correct!?!?
    ...
    # parse_end_instructionst :: [Token] -> [[Token]]
    def parse_end_instructions(self, tokens : List[Token]) -> Union[List[List[Token]], None]:
        if tokens == []:
            return []
        head = self.parse_end_instruction(tokens)
        tail = tokens[len(head):]
        return [head] + self.parse_end_instructions(tail)

    # returns one list of tokens of a input list of tokens
    # return list will end on a "end token"
    # parse_end_instructionst :: [Token] -> [Token]
    def parse_end_instruction(self, tokens : List[Token]) -> Union[List[Token], None]:
        if tokens == []:
            return [] # error: no ending of instruction??
        last_token = ['CALL', 'INTERGER', 'IDENTIFIER', 'INPUT', 'PRINT_WO_NL', 'RPAREN', 'LINE_DOWN', 'END']
        head, *tail = tokens
        if head.name in last_token:
            return [head]
        else:
            return [head] + self.parse_end_instruction(tail)
...
'''
    ...
    # parse_variable :: [Token] -> [Token]
    def parse_variable(self, tokens : List[Token]) -> List[Token]:
        if tokens == []:
            return []
        head, *tail = tokens
        if head.name in ['INTERGER', 'INPUT', 'IDENTIFIER', 'CALL']:
            return [head] + self.variable(tail)
        else:
            return []
...
# define or modify variable
if first_token.name == 'VARIABLE':
    identifier, *tail = tail
    if identifier.name == 'IDENTIFIER':
        operator, *tail = tail
        if operator.name in ['ASSIGN', 'ADD', 'MIN']:
            value, *tail = tail
            if value.name in ['INTERGER', 'INPUT']:
                i = Instruction([first_token, identifier, operator, value])]
                return [i] + self.parse(tail)
            elif value.name == 'CALL':
                operator_2, *tail = tail
                if operator_2.name == 'IDENTIFIER':
                    value_2, *tail == tail
                    if value_2.name
'''
